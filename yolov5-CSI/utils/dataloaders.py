...
class LoadStreams:
    ...
    def __init__(self, sources='file.streams', img_size=640, stride=32, auto=True, transforms=None, vid_stride=1):
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            self.imgs[i] = picam2.capture_array("main")  # guarantee first frame
            self.imgs[i] = cv2.cvtColor(self.imgs[i], cv2.COLOR_BGR2RGB)
            self.threads[i] = Thread(target=self.update, args=([0, picam2, s]), daemon=True)
    ...
    def update(self, i, picam2, stream):
